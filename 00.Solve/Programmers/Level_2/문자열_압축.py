def solution(s):
    ...
    for i in range(window, 0, -1):
        compression = 0
        temp = s[:i]  # 현재 탐색중인 끊는 단위만큼 끊은 문자열 담아줄 거임
        count = 1  # 끊은 문자열이 몇 번 연속으로 등장했는지 담아줄 거임
        for j in range(i, len(s), i):
            if temp == s[j:j + i]:  # 끊은 문자열과 같은 문자열을 만나면
                count += 1  # 등장 횟수 1 증가
            else:  # 다른 문자열을 만나면 이전에 쌓아둔 문자열 압축 후 카운트 초기화
